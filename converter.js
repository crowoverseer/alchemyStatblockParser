...
const ABILITY_REGEX = /STR[\s]*(?<str>\d+)[\s()\+\-\d]*?DEX[\s]*(?<dex>\d+)[\s()\+\-\d]*?CON[\s]*(?<con>\d+)[\s()\+\-\d]*?INT[\s]*(?<int>\d+)[\s()\+\-\d]*?WIS[\s]*(?<wis>\d+)[\s()\+\-\d]*?CHA[\s]*(?<cha>\d+)/ig
const RETURN = '[r]'
...
const getAbilityBonus = ability =>
  getAbilityBonusByScore( npc.abilityScores.find( abilItem => abilItem.name === ability ).value )
...
const parseAC = () => {
  const armorClass = findAndShift(
    /^Armor Class.*?\(.*?(?<ac>\d+)/i, // alternate armor class
    "ac"
  ) || findAndShift(
    /^Armor Class\s?(?<ac>\d+)/i,
    "ac"
  )
...
const parseSpeed = () => {
  ...
  speeds.forEach( (speedStr, idx) => {
    ...
    if( !gotWalk && ( /Walk/i.test(mode) || idx == speeds.length - 1 ) ) {
      gotWalk = true;
      defaultSpeed = speed;
    }
  })
...
const parseAbilityScores = () => {
  const { str, dex, con, int, wis, cha } = findAndShift(
    ABILITY_REGEX,
...
const parseChallenge = () => {
  const challenge = findAndShift(
    /Challenge\s*(rating)?\s*?(?<challenge>[\d\/]+)\s*?\(([\d,]+)\s?XP\)/i, "challenge"
  )
...
const createSkillProficiencies = (profarr) => {
  ...
  Object.keys(skillAbilityTable).forEach( skillName => {
    ...
    const skillProf = {
      "abilityName": abName,
      "name": skillName,
      "proficient": getAbilityBonus(abName) < Number( (profarr.find(skill => skill.name == skillName) || {}).value || -10 )
    }
...
const parseSkills = () => {
  ...
  const skillsrow = findAndShift(
    /^Skills(?<skillsrow>.*)/i, "skillsrow"
  ) || ""
...
const parseSenses = () => {
  ...
  if( sensesline ) {
    ...
    const sensesObjectsArray = senses.map( sense => {
...
const parseConditionImmunities = () => {
  ...
  if( !immunitiesLine )
    return;
  const immunities = immunitiesLine.split(',').map(immunity => capitalize( immunity.trim() ))
  npc.conditionImmunities = immunities
}

const parseDescription = () => {
  let descriptionSeparatorPos = -1;
  if( spaceDescr ) {
...
const fillNPCSpells = () => {
  ...
  knownSpells.forEach( spellName => {
    ...
    for( let i = 0; i < spellDb.length; i++ ) {
      const rgx = new RegExp( spellName
        .replace(/\(.*?\)/gi,'')
        .replace(' ', '\\s'),
        'i'
      )
      if( rgx.test( spellDb[i].name ) ) {
        const { name, castingTime, components, description, duration, level, range, savingThrow, school, tags } = spellDb[i]
        npc.spells.push({
          name, castingTime, components, description, duration, level, range, savingThrow, school, tags
        })
        // if some info about slots is exists, but not complete
        if( level && knownSlots.length ) {
          if( knownSlots[level] ) {
            spellSlots[level-1].max = spellSlots[level-1].remaining = knownSlots[level];
          } else {
...
const parseSpellListAt = (listIdx) => {
  const isSpellListPattern = '^(At\\swill|\\d\\s?\\/\\s?day|\\d(nd|st|rd|th)\\s?(-|–)\\s?\\s*level|Cantrips|\\d(nd|st|rd|th)\\s?(-|–)\\s?\\d(nd|st|rd|th)\\s*level|\\d(nd|st|rd|th)\\s+level)'
  ...
  const spellForLevelRegexps = [
    ...
    new RegExp( isSpellListPattern.concat('.*?:\\s*\\((?<slotcount>\\d+)\\s*(?<slotlevel>\\d+).*?\\)\\s*:\\s*(?<spells>.*)') ),
  ...
  while( isSpellListRegexp.test(source[cursor]) ) {
    for( let i = 0; i < spellForLevelRegexps.length; i++ ) {
      ...
      if( regResult ) {
        ...
        result.push( spells )
        spells.replace(/\(.*?\)/gi,'').split(',').forEach( spell => {
          knownSpells.push( spell.replace( /\(.*\)/i, '').replace('*','').trim() )
        })
        break;
      }
    }
...
const parseSpells = (from = 0, shift = 0) => {
  ...
  for( let i = from; i < source.length; i++ ) {
    ...
    if( regRes ) {
      innateSpellsIndex = i;
      const { ability } = /(spellcasting\s+ability\s+is)\s*(?<ability>\w+)/.exec( source[i])?.groups || {};
      if( ability ) {
  ...
  if( spellLists.length ) {
    const protectedSpells = source.slice( innateSpellsIndex, innateSpellsIndex + spellLists.length + 1 + shift).join(RETURN).replace(/\n/gi, '')
    source.splice( innateSpellsIndex, spellLists.length + 1 + shift, protectedSpells )
...
const parseLedendaryActions = () => {
  ...
  source = [
    `Legendary Actions. ${actionDescription}`,
    ...source
  ]
...
const parseActions = () => {
  ...
  const damageRegex = /\(\s*(?<dice>\d+d\d+)(\s?\+?\s?(?<bonus>\d+))?\s*\)\s*?(?<type>\w+)\s*\s+damage/gi
  const savingThrowRegex = /DC\s*(?<dc>\d+)\s*(?<ability>\w+)\s*saving\sthrow/i
  const actions = actionsLines.filter(action => action).map( (action, idx) => {
    ...
    if( name && description ) {
      ...
      if( savingThrowRegexResult ) {
        const { dc, ability: abilitySaveFull } = savingThrowRegexResult.groups;
        savingThrow = {
      ...
      if( attackRegexResult ) {
        ...
        // checking ability
        const strBaseAttack = getAbilityBonus("str") + getProfByChallenge( npc.challengeRating );
        const dexBaseAttack = getAbilityBonus("dex") + getProfByChallenge( npc.challengeRating );
        let ability = "str"
        if( bonus === strBaseAttack )
          ability = "str"
        const isRanged = rangeType.toUpperCase() === 'RANGED'
        const [ normalRange, longRange ] = (range || ["30/60"]).split("/")
        if( bonus === dexBaseAttack && !( strBaseAttack === dexBaseAttack && isRanged === false ) )
          ability = "dex"
        ...
        attack = {
          ...
          isProficient: true,
          isRanged,
          ...( isRanged ? { range: Number( normalRange ), longRange: Number( longRange) } : {} ),
          name,
          rollsAttack,
          savingThrow,
        }
...
try {
  ...
  try {
    ...
    // STR DEX CON
    // 12 (+1) 12 (+1) 13 (+1)
    const brokenAbilityRegexp = /STR\s*DEX\s*CON\s*?\n(?<str>\d+)\s*\(.*?\)\s*(?<dex>\d+)\s*\(.*?\)\s*(?<con>\d+)\s*/gi
